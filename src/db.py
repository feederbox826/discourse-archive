...
def add_topic(topic):
  cur.execute('''
    INSERT OR REPLACE INTO topics (id, slug, created_at, updated_at, highest_post_number)
    VALUES (?, ?, ?, ?, ?);
  ''', (topic['id'], topic['slug'], topic['created_at'], topic['last_posted_at'], topic['highest_post_number']))
  conn.commit()
